...
andi $t1,$t0,1
li $t2,0  #counter
bne $t1,$0,odd2   #check odd or even
even:
    beq $t2,$t0,evenend #even increment
    addi $a0,$a0,4
    addi $t2,$t2,2
	j even
evenend:
...
odd2:addi $t0,$t0,-1
 odd:   beq $t2,$t0,oddend   #odd increment
    addi $a0,$a0,4
    addi $t2,$t2,2
        j odd
oddend:
...
strategy:  # a0 = myHand, a1 = theirHand
   ...
   move $a0,$t0
   sw $t0,12($sp)
   li $a1,1  #check for faec value
 jal hw1
    
   bne $v0,1,face_zero_min_one
   li $v0,1
   j end1
...
face_zero:
 ...
 jal hw1
   ...
   bne $v0,1,facezero_zero_min_one
   li $v0,2
   j end1

facezero_zero_min_one:
   bne $v0,-1,zero_zero
   li $v0,1
   j end1
